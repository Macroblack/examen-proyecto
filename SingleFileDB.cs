...
namespace DiaryJournal.Net
{
    ...
    public static class SingleFileDB
    {
        ...
        public static bool CreateLoadDB(String file, String dbName, ref SingleFileDBContext ctx, bool overwrite, bool create)
        {
            ...
            if (!create)
            {
                // if creation is not demanded, so if the file does not exists, we just cannot create new.
                if (!File.Exists(file))
                    return false;
            }
            else
            {
                // create is demanded so if file does not exists create it
                if (!File.Exists(file))
                {
                    if (!SingleFileDB.createDb(file))
                        return false;
            ...
            if (!SingleFileDB.initDB(ctx, file, dbName))
                return false;
        ...
        public static bool createDb(String file)
        {
            ...
            try
            {
                ...
                String dbIndexingFile = file + DatabaseIndexing.dbIndexingFileName; // Path.Combine(dbBasePath, DatabaseIndexing.dbIndexingFileName);
        ...
        public static bool initDB(SingleFileDBContext ctx, String file, String dbName)
        {
            ...
            if (result)
            {
                ...
                ctx.dbIndexingFile = file + DatabaseIndexing.dbIndexingFileName; // Path.Combine(dbBasePath, DatabaseIndexing.dbIndexingFileName);
        ...
        public static bool autoLoadCreateDefaultDB(ref SingleFileDBContext ctx, myConfig cfg, bool overwrite)
        {
            ...
            try
            {
                ...
                // initialize all tables
                if (!SingleFileDB.initDB(ctx, file, defaultDBFileName))
                    return false;
        ...
        public static bool deleteDB(ref SingleFileDBContext ctx)
        {
            String dbPath = ctx.dbpath;
            String dbConfigFile = ctx.dbConfigFile;
            String dbIndexingFile = ctx.dbIndexingFile;
        ...
        public static bool CopyDB(ref SingleFileDBContext ctx, String destBasePath, bool copy, bool overwrite)
        {
            String dbPath = ctx.dbpath;
            String dbConfigFile = ctx.dbConfigFile;
            String dbIndexingFile = ctx.dbIndexingFile;
            ...
            FileInfo srcinfo = new FileInfo(dbPath);
            FileInfo srcConfigFileInfo = new FileInfo(dbConfigFile);
            FileInfo srcIndexingFileInfo = new FileInfo(dbIndexingFile);

            String destDBFile = Path.Combine(destBasePath, srcinfo.Name);
            String destDBConfigFile = Path.Combine(destBasePath, srcConfigFileInfo.Name);
            String destIndexingFile = Path.Combine(destBasePath, srcIndexingFileInfo.Name);

            // now copy files
            try
            {
                File.Copy(dbPath, destDBFile, overwrite);
                File.Copy(dbConfigFile, destDBConfigFile, overwrite);
                File.Copy(dbIndexingFile, destIndexingFile, overwrite);
            }
            catch
            {
                return false;
            ...
            String title = "", String rtf = "", bool newId = true, bool checkpoint = true)
        {
            ...
            if (DBImport)
            {
                if (!createNode(ctx, ref node, rtf, resetCD, resetMD, resetDD, newId, checkpoint))
                    return null;
            }
            return node;
        }
        ...
        public static bool updateDBChapterData(SingleFileDBContext ctx, ref ChapterData chapterData, bool checkpoint = true)
        {
            // found chapter data object, so update
            try
            {
                ctx.chapterDataCollection.Update(chapterData);
                if (checkpoint) ctx.db.Checkpoint();
                return true;
            }
            catch (Exception)
            {
        ...
        public static bool updateNode(SingleFileDBContext ctx, ref myNode node)
        {
            ...
            return updateDBChapter(ctx, ref node.chapter, checkpoint);
        }
        */
        ...
        public static bool updateDBChapter(SingleFileDBContext ctx, ref Chapter dbChapter, bool checkpoint = true)
        {
            try
            {
        ...
        public static bool UpdateChapterAndData(SingleFileDBContext ctx, ref Chapter dbChapter, String? rtf, bool checkpoint = true)
        {
            if (rtf != null)
            {
            ...
            return updateDBChapter(ctx, ref dbChapter, checkpoint);
        }
        ...
        public static bool purgeDBDataNode(SingleFileDBContext ctx, ref myNode node, bool checkpoint = true)
        {
            if (node == null)
                return false;
        ...
        public static bool purgeNode(SingleFileDBContext ctx, ref myNode node, bool checkpoint = true)
        {
            if (node == null)
                return false;

            // first purge this chapter's data
            if (!purgeDBDataNode(ctx, ref node, checkpoint))
                return false;
