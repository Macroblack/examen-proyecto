...
namespace DiaryJournal.Net
{
    public static class entryMethods
    {
        ...
        public static bool GenerateLineagePath(ref List<myNode> allNodes, ref myNode srcNode, out String outFormatted, out List<myNode> outLineage)
        {
            ...
            // get lineage
            lineage = findBottomToRootNodesRecursive(ref allNodes, ref srcNode, true, true, true);
        ...
        public static List<myNode> findBottomToRootNodesRecursive(ref List<myNode> allNodes, ref myNode srcNode, bool includeDeleted = false,
        ...
        public static int IsAncestorNode(ref List<myNode> allNodes, Int64 nodeToCheckId, myNode srcNode, bool deleted = false)
        {
            ...
            myNode? node = srcNode;
            List<myNode> ancestors = findBottomToRootNodesRecursive(ref allNodes, ref srcNode, deleted, true, true);
            ...
            
        {
            ...
            if (!mark)
            {
                ...
                List<myNode> effectedNodes = findBottomToRootNodesRecursive(ref allNodes, ref node, true, true, true);
            ...
            bool writeDBIndexingFile = true, bool checkpoint = true, bool buildLineageList = true)
        {
            ...
            if (entryMethods.DBCreateNode(ref cfg, ref clone, rtf, false, false, false, true, writeDBIndexingFile, checkpoint))
            {
                ...
                if (buildLineageList)
                    clone.lineage = entryMethods.findBottomToRootNodesRecursive(ref allNodes, ref clone, false, true, true);
        ...
        public static void DBReloadLineages(ref List<myNode> allNodes, bool deleted = false,
            bool topToBottom = false, bool includeNonDeleted = false)
        {
            ...
            foreach (myNode? node in allNodes)
            {
                ...
                listedNode.lineage = entryMethods.findBottomToRootNodesRecursive(ref allNodes, ref listedNode, deleted, topToBottom, includeNonDeleted);
            }
            ...
            ref mySystemNodes? systemNodesOut)
        {
            ...
            myNode? journalNode = systemNodes.getSystemNode(NodeType.Journal);
            ...
            if (yearNodes.Count() <= 0)
            {
                ...
                yearNode.chapter.parentId = journalNode.chapter.Id;
                ...
                // lineage
                yearNode.lineage = new List<myNode>(journalNode.lineage);
                yearNode.lineage.Add(yearNode);
                systemNodes.YearNodes.Add(yearNode);
            }
            else
            {
            ...
            if (monthNodes.Count() <= 0)
            {
                ...
                // lineage
                monthNode.lineage = new List<myNode>(yearNode.lineage);
                monthNode.lineage.Add(monthNode);
                systemNodes.MonthNodes.Add(monthNode);
            }
            else
            {
        ...
        public static bool ImportSet(FrmJournal? parentForm, ref myConfig cfgSrc, ref myConfig cfgDest, String dbName, ref List<myNode> allNodes,
        ...
        // this method is for upgrading old past version db
        public static bool DBFixUpgradeDBIndexingFile(String file)
        {
            try
            {
                byte[] bytes = File.ReadAllBytes(file);
                Int64 value = BitConverter.ToInt64(bytes, 0);
                String newFile = Path.ChangeExtension(file, DatabaseIndexing.dbOpenDBIndexingExtension);
                File.Move(file, newFile);
                File.WriteAllText(newFile, value.ToString());
            }
            catch (Exception e)
            {
                return false;
