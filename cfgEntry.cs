...
namespace DiaryJournal.Net
{
    public static class cfgEntry
    {
        ...
        public static bool fromCfg(ref Chapter chapter, String file)
        {
            ...
            Int32 caretIndex = 0, caretSelectionLength = 0;
            Int32 documentWidth = myConfig.defaultDocumentWidth;
            ...
            try
            {
                caretIndex = Int32.Parse(values[14]);
                caretSelectionLength = Int32.Parse(values[15]);
            }
            catch { }
            try
            {
                documentWidth = Int32.Parse(values[16]);
            }
            catch { }
            ...
            chapter.caretIndex= caretIndex;
            chapter.caretSelectionLength= caretSelectionLength;
            chapter.documentWidth = documentWidth;
            return true;
        }
        ...
        public static String toCfg(ref Chapter chapter)
        {
            ...
            body += String.Format(@"{0}:::" + Environment.NewLine, chapter.caretIndex.ToString());
            body += String.Format(@"{0}:::" + Environment.NewLine, chapter.caretSelectionLength.ToString());
            body += String.Format(@"{0}:::" + Environment.NewLine, chapter.documentWidth.ToString());
            return body;
        }
    }
}
