...
namespace DiaryJournal.Net
{
    public class textFormatting
    {
        ...
        public void formatInsertString(AdvRichTextBox rtb, String text)
        {
            rtb.SelectedText = text;
        }
        public void formatInsertString(WpfRichTextBoxEx rtb, String text)
        {
            rtb.Focus();
            TextSelection selection = ((rtb.IsSelectionActive) ? rtb.Selection : ((rtb.sel != null) ? rtb.sel : rtb.Selection));
            if (selection == null) return;

            selection.Start.GetInsertionPosition(LogicalDirection.Forward).InsertTextInRun(text);
        }
