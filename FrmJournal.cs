...
namespace DiaryJournal.Net
{
    ...
    public partial class FrmJournal : Form
    {
        ...
        private void FrmJournal_Load(object sender, EventArgs e)
        {
            ...
            tvEntries.ImageList.Images.Add(DiaryJournal.Net.Properties.Resources.template1);
            ...
            this.Text = "" + this.Text + " Version " + Application.ProductVersion + ", uses .Net 8.0, Compiled/Built on: " + strBuildDateTime;
        ...
        private void TvEntries_DragDrop(object? sender, DragEventArgs e)
        {
            ...
            if (!draggedTreeNode.Equals(targetTreeNode) && !ContainsNode(draggedTreeNode, targetTreeNode))
            {
                ...
                if (e.Effect == DragDropEffects.Move)
                {
                    ...
                    // rebuild lineage
                    draggedNode.lineage = entryMethods.findBottomToRootNodesRecursive(ref allNodes, ref draggedNode, false, true, true);
                }
        ...
        public void setupTemplateEntry()
        {
            if (!cfg.ctx0.isDBOpen() && !cfg.ctx1.isDBOpen())
                return;
            ...
            //    parentId = Int64.Parse(tvEntries.SelectedNode.Name);

            string? input = "";
            if (userInterface.ShowInputDialog("input title for template entry (mandatory)", ref input) != DialogResult.OK)
                return;

            // template must have a name/title
            if (input.Length <= 0) return;

            setupNewSpecialEntry(DateTime.Now, parentId, NodeType.Template, NodeType.Templates, input);
        }
        public void setupNewSpecialEntry(DateTime dateTime, Int64 parentId = 0, NodeType nodeType = NodeType.Template, NodeType parentSystemNodeType = NodeType.Templates,
            String title = "")
        {
            if (!cfg.ctx0.isDBOpen() && !cfg.ctx1.isDBOpen())
                return;
            ...
            myNode? parentSystemNode = systemNodes.getSystemNode(parentSystemNodeType);
            ...
            else if (parentId == 0)
            {
                if (parentSystemNode != null)
                    parentId = parentSystemNode.chapter.Id;
                else
                    return; // abort error
            }
            else
            ...
            List<myNode> lineageChain = entryMethods.findBottomToRootNodesRecursive(ref allNodes, ref node, false, true, true);
            ...
            this.Invoke(loadRootToBottomNodes, node.lineage, false);
            ...
            String title = "")
        {
            ...
            // reload lineages
            //entryMethods.DBReloadLineages(ref allNodes, true, true, true);
        ...
        public void reloadAll(bool autoAllNodes, bool autoSystemNodes, bool autoTreeView, bool autoLineage, ref mySystemNodes? systemNodesOut)
        {
        ...
        */
        ...
        public TreeNode? __initTreeViewEntry(ref myNode nodeEntry)
        {
            ...
            entryMethods.loadNodeTreeViewItemIcon(newNode, nodeEntry);
        ...
        private void toolStripMenuItem19_Click(object sender, EventArgs e)
        {
            ...
            if (MessageBox.Show("warning: are you sure you want to overwrite and load a new db? this will overwrite (destroy) current db!", "warning", MessageBoxButtons.YesNo, MessageBoxIcon.Warning) != DialogResult.Yes) return;
        ...
        public void purgeTrashCan()
        {
            ...
            foreach (System.Windows.Forms.ListViewItem listViewItem in lvTrashCan.CheckedItems)
            {
                ...
                entryMethods.DBDeleteOrPurgeNodeRecursive(cfg, ref allNodes, ref node, false, true, false);
            }
        ...
        public void restoreTrashCan()
        {
            ...
            foreach (System.Windows.Forms.ListViewItem listViewItem in lvTrashCan.CheckedItems)
            {
                ...
                entryMethods.DBDeleteOrPurgeNodeRecursive(cfg, ref allNodes, ref node, false, false, false);
            }
        ...
        public void promoteNode()
        {
            ...
            // rebuild lineage
            node.lineage = entryMethods.findBottomToRootNodesRecursive(ref allNodes, ref node, false, true, true);
        ...
        public void __selectSearchNode()
        {
            ...
            foreach (myNode node in selNodes)
            {
                ...
                foreach (ListViewItem listedItem in lvSearchWhere.Items)
                {
                    ...
                    if (entryMethods.IsAncestorNode(ref allNodes, listedItemId, node, false) == 1)
        ...
        private void nullifyemptyAllFontAndColorEntireDbToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            // process
            long processed = 0;
            entryMethods.DBNullOrEmptyNodes(ref cfg, ref allNodes, out processed, formOperation);
        ...
        public bool NullOrEmptyNodesDB(DatabaseType srcDBType)
        {
            // firstly save entry
            saveEntry();
        ...
        private void nullemptyAllNodesInADbToolStripMenuItem_Click(object sender, EventArgs e)
        {
            FormSelectDB form = new FormSelectDB();
        ...
        public void FixUpgradeDBIndexingFile()
        {
            // firstly save entry
            saveEntry();

            OpenFileDialog ofd = new OpenFileDialog();
            ofd.Filter = "database indexing files *.*|*.*";
            ofd.FilterIndex = 1;
            if (ofd.ShowDialog(this) != DialogResult.OK)
                return;

            entryMethods.DBFixUpgradeDBIndexingFile(ofd.FileName);
        }

        private void step1FixUpgradeDbConfigToolStripMenuItem_Click(object sender, EventArgs e)
        {
            FormSelectDB form = new FormSelectDB();
            if (form.ShowDialog(this) != DialogResult.OK) return;

            FixUpgradeDBIndexingFile();

        }

        private void step2FixUpgradePreviousVersionOpenFilesystemDbToolStripMenuItem_Click(object sender, EventArgs e)
        {
        ...
        private void toolStripMenuItem82_Click(object sender, EventArgs e)
        {
            ...
            myNode? node = form.outSelectedNode;
            String rtf = entryMethods.DBLoadNodeData(cfg, node.chapter.Id, node.DirectorySectionID);

            // initialize an rtb control and set the rtf into it for processing
            //System.Windows.Controls.WpfRichTextBoxEx rtb = new System.Windows.Controls.WpfRichTextBoxEx();
            //rtb.Rtf = rtf;
            ...
            foreach (Object? item in codes)
            {
                if (item == null) continue;
                TemplateFormat.TemplateCodeItem code = (TemplateFormat.TemplateCodeItem)item;
                cfg.templateFormat.transform(rtb, code);
            }

            // finally insert the transformed rtf into our primary rtb control at the selection location
            rtf = WpfRtbMethods.ToRtf(rtb);
            rtbEntry.SelectedRtf = rtf;
            MessageBox.Show("template inserted", "done", MessageBoxButtons.OK, MessageBoxIcon.Information);
        }

        private void findAvailableDbIndexToolStripMenuItem_Click(object sender, EventArgs e)
        {
            // first save the entry
            this.Invoke(saveEntry);
            ...
            List<myNode> allNodes = entryMethods.DBFindAllNodes(cfg, false, false);
            Int64 maxIndex = 1;
            myNode? node = allNodes.MaxBy(x => x.chapter.Id);
            if (node != null) maxIndex = node.chapter.Id + 1;

            formOperation.close();

            MessageBox.Show("available index: " + maxIndex, "done", MessageBoxButtons.OK, MessageBoxIcon.Information);
        }
        ...
        {
            FormSelectDB form = new FormSelectDB();
            form.Text = "select source db";
            if (form.ShowDialog(this) != DialogResult.OK) return;

            FormSelectDB form2 = new FormSelectDB();
            form2.Text = "select destination cloned db";
            if (form2.ShowDialog(this) != DialogResult.OK) return;

            // now clone source to destination db
            CloneDB(form.selectedDBType, form2.selectedDBType);
        }
